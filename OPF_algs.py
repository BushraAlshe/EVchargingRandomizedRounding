    ...
    for size in np.arange(1, max_guessed_set_size + 1):
        for guess_I1 in itertools.combinations(enumeration_set, size):
            ...
            if sol.succeed:
                ...
                print 'obj', best_sol.obj, '| frac_obj', solf.obj, "| acheived e", 1 - (
                        (1 + epsilon) * solf.obj) / best_sol.obj
...
def round_EV_scheduling4(ins, guess_x={}, round_x_after_y=True):
    ...
    if sol.succeed:
        ...
        charging_cost = {k: sol.x[(k, c, t)] * ins.cost_rate_matrix[c, t] for k in customers for t in
                         ins.customer_charging_time_path[k] for c in ins.customer_charging_options[k]}
        ...
        for k in sorted_customers:
            time_list_cost = {(c, t): sol.x[k, c, t] for c in ins.customer_charging_options[k] for t in
                              ins.customer_charging_time_path[k]}
            sorted_time_option = {r: v for r, v in
                                  sorted(time_list_cost.iteritems(), key=lambda (r, v): (v, r), reverse=True) if v != 0}
...
def round_EV_scheduling2(ins, guess_x={}, round_x_after_y=True):
    ...
    if sol.succeed:
        ...
        for t in np.arange(ins.scheduling_horizon):
            ...
            list_x = {(k, c): sol.x[(k, c, t)] for k in ins.customers_at_time[t] for c in
                      ins.customer_charging_options[k]}
            ...
            for (k, c) in sorteted_ev_option_pair:
                if ins.rounding_tolerance < sol.x[(k, c, t)] < 1 - ins.rounding_tolerance:
                    ...
                    usage_after_rounding_down = energy_usage[k] - sol.x[k, c, t] * ins.charging_rates[
                        c] * ins.step_length
...
def round_EV_scheduling1(ins, guess_x={}, round_x_after_y=True):
    ...
    if sol.succeed:
        ...
        for t in np.arange(ins.scheduling_horizon):
            ...
            sorted_customers = [k for k, v in sorted(cust_x.iteritems(), key=lambda (k, v): (v, k), reverse=True) if
                                v not in [0, 1]]
            # print 'customers at time ',t, ins.customers_at_time[t]
...
def round_EV_scheduling_fixed_interval(ins, guess_x={}, round_x_after_y=True):
    ...
    if sol.succeed:
        ...
        ww = 1 - delta + ((1 + (delta ** (1.0 / 3.0))) ** 3.0)
        ww = ww * 3
        alpha_delta = (1 - delta) / ww
        #alpha_delta = random.uniform(0, 1)
        print(delta)
        customers_heavy = []
        customers_light = []
        for k in customers:
            ...
            w = 0
            for t in ins.customer_charging_time_path[k]:
                w = w + ins.charging_rates[1]
            # print (w)
            if w > delta * ins.capacity_over_time[t]:
                customers_heavy.append(k)
            else:
                customers_light.append(k)
        print(customers_heavy)
        print(customers_light)
        print(len(customers_heavy))
        print(len(customers_light))
        obj1 = 0
        # randomized rounding. #for the light only
        enumeration_set = []
        # max_guessed_set_size = int(1.0 / delta)
        max_guessed_set_size = 0
        counter = 0
        for k in customers:
            # rounding y
            if k in customers_heavy:
                counter = counter + 1
                if counter <= max_guessed_set_size:
                    enumeration_set.append(k)

        for k in customers:
            for c in ins.customer_charging_options[k]:
                # rounding y
                if k in customers_light:
                    rand = random.uniform(0, 1)
                    # print rand
                    if rand <= alpha_delta * sol.x[k, c]:  # Flooring (18)
                        print(rand)
                        print(alpha_delta)
                        sol.x[k, c] = 0
                        sol.frac_x_comp_count += 1
                elif k in enumeration_set:
                    sol.x[k, c] = 0

                # calculate new obj
                obj1 += ins.customer_utilities[k] * sol.x[k, c]
        ...
        # enumeration_set = []
        customers = np.arange(ins.n)
        max_guessed_set_size = 1
        # This loop is to create the enurmation set and set the light users to 0.
        counter = 0
        for k in customers:
            for c in ins.customer_charging_options[k]:
                # rounding y
        ...
        # max_guessed_set_size = int(1.0/delta)
        print(enumeration_set)
        max_obj2 = 0
        # does the enumeration, calcultes max_obj2 for the whole set.
        for size in np.arange(1, max_guessed_set_size + 1):
            for guess_I1 in itertools.combinations(enumeration_set, size):
                obj2 = 0
                # print(guess_I1)
                guess_x = {v: 1 for v in guess_I1}
                guess_I2 = {l: 0 for l in np.setdiff1d(customers_heavy, guess_I1)}
                # print(guess_x)
                # print(guess_I2)
                for o in guess_x:
                    for c in ins.customer_charging_options[o]:
                        sol.x[o, c] = 1
                        sol.frac_x_comp_count += 1
                for p in guess_I2:
                    for c in ins.customer_charging_options[p]:
                        sol.x[p, c] = 0
                        sol.frac_x_comp_count += 1
                    # calculate new obj
                for ff in np.arange(ins.n):
                    for gg in ins.customer_charging_options[ff]:
                        obj2 += ins.customer_utilities[ff] * sol.x[ff, gg]
                        # print(sol.x[ff, gg])
                if obj2 > max_obj2:
                    max_obj2 = obj2
        print (obj1)
        print(max_obj2)
        obj = max(obj1, max_obj2)
        sol.running_time = time.time() - t1
        ...
        for t in np.arange(ins.scheduling_horizon):
            ...
            for (k, c) in ins.customers_at_time[t]:
                P[t] += sol.x[(k, c)] * ins.charging_rates[1]
...
# for e-energy 2018
def max_ev_scheduling_OPT_fixed_interval(ins, guess_x={}, fractional=False, debug=False, tolerance=0.001,
                                         alg="min_OPF_OPT"):
    t1 = time.time()
    ...
    obj = gbp.quicksum(
        x[k, c] * ins.customer_utilities[k] for k in range(ins.n) for c in ins.customer_charging_options[k])
    m.setObjective(obj, gbp.GRB.MAXIMIZE)
    m.update()
    ...
    for t in time_path:
        ...
        ev_charge_list = [x[(k, c)] * (ins.charging_rates[1]) for (k, c) in ins.customers_at_time[t] if
                          len(ins.customers_at_time[t]) != 0]
        # print(ins.customers_at_time)
    ...
    # chargers physical limitation. (Max number of chargers in each station.
    for t in time_path:
        #print("t")
        #print(t)
        ...
        for b in np.arange(3):
            ...
            # print(b)
            count_cs = 0
            #print(ins.customers_at_time[t])

            P_EV = [x[(k, c)] for (k, c) in ins.customers_at_time[t]
                    if len(ins.customers_at_time[t]) != 0 and (c == b)]  # list of charging rates used at the time
            #print(P_EV)
            count_cs = count_cs + len(P_EV)
            #print(count_cs)
            # print(count_cs)
            m.addConstr(gbp.quicksum(P_EV), gbp.GRB.LESS_EQUAL, ins.charging_stations_chargers[b],
                        "P[t,c] <= charging rate* N_c" % (k))
    ...
    # for t in time_path:
    print(sol.x)
    # print(sol.P[t] - ins.base_load_over_time[t])
    # print(ev_charge_list)
    # print(ins.customers_at_time)
    # print(ins.base_load_over_time[t])
...
def min_OPF_OPT(ins, guess_x={}, epsilon=None, fractional=False, debug=False, tolerance=0.001, alg="min_OPF_OPT"):
    ...
    if u.gurobi_handle_errors(m, algname=alg):
        ...
        sol.int_comp_count = len(
            [k for k in ins.I if sol.x[k] >= 1 - ins.rounding_tolerance or sol.x[k] <= ins.rounding_tolerance])
