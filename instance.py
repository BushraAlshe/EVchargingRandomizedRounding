...
class OPF_EV_instance(object):
    def __init__(self):
        ...
        self.customer_usage = list(range(1000))
...
def sim_instance_ev_scheduling_fixed_intervals(scenario="L", n=10,
                                               time_steps=48, step_length=.25, num_base_load_household=650,
                                               capacity=1000000,
                                               charging_rates=[1500, 7000, 50000],
                                               charging_rates_stds=[500, 1000, 10000],
                                               initial_SOC_range=(.2, .8), SOC_mu=.5, SOC_std=.3,
                                               battery_size_range=(24000., 100000.), battery_size_mu=30000.,
                                               battery_size_std=10000.,
    ...
    ins.charging_stations_chargers = [1, 1, 1]
    ...
    Minutes = list(range(1000))
    Hours_done = list(range(1000))
    Minutes_done = list(range(1000))
    doneSession_done = list(range(1000))
    chg_length = list(range(1000))
    Available_time = list(range(1000))
    ...
    # 1/3/2011 - 1/4/2011
    day1 = np.array(
        [0.717, 0.637, 0.619, 0.593, 0.604, 0.654, 0.747, 0.809, 0.798, 0.778, 0.782, 0.804, 0.828, 0.788, 0.760, 0.767,
         0.843, 1.083, 1.173, 1.200, 1.170, 1.099, 0.961, 0.815])
    day2 = np.array(
        [0.713, 0.657, 0.630, 0.599, 0.616, 0.676, 0.773, 0.827, 0.763, 0.720, 0.709, 0.679, 0.691, 0.676, 0.664, 0.658,
         0.738, 0.972, 1.076, 1.118, 1.110, 1.049, 0.908, 0.785])
    ...
    # cost=level1_cost
    cost = np.repeat(cost, 1 / step_length, axis=0)
    ...
    for k in np.arange(n):
        ...
        ignored_values = set(["null", "", None])  # cleaning the data as some contain null inputs for user inputs
        data = [elem for elem in data['_items'] if elem['doneChargingTime'] and elem['userID'] not in ignored_values]
        Hours[k] = json.dumps(data[k]["connectionTime"][17:19])
        Hours[k] = Hours[k].strip('""')
        Hours[k] = float(Hours[k])
        Minutes[k] = json.dumps(data[k]["connectionTime"][20:22])
        Minutes[k] = Minutes[k].strip('""')
        Minutes[k] = float(Minutes[k])

        ins.customer_usage[k] = (float(json.dumps(data[k]["userInputs"][0]["kWhRequested"])) * 1000) / (
                (float(json.dumps(data[k]["userInputs"][0]["minutesAvailable"]))) / 60)

        ins.customer_charge_start_at_time[(k)] = int(
            (int((Hours[k] / step_length)) + (Minutes[k] + 15) / 15))  # 15 MINUTES

        chg_length = ins.customer_usage[k] / (float(ins.charging_rates[1]) * ins.step_length)  # in time steps
        ins.customer_charging_length[k] = np.floor(chg_length)

        # Available_time[k] = int(data[k]["userInputs"][0]["minutesAvailable"]) / 15  # in time steps

        chg_path_nodes = np.arange(ins.customer_charge_start_at_time[k],
                                   ins.customer_charge_start_at_time[k] + ins.customer_charging_length[k] + 1)
        ...
        for c in ins.customer_charging_options[k]:
            ...
            for t in ins.customer_charging_time_path[k]: ins.customers_at_time[t].append((k, c))
            ins.customer_charging_cost[k] = np.sum(
                ins.cost_rate_matrix[t] for t in ins.customer_charging_time_path[k])
            # print(ins.customer_charging_cost[k])
            if scenario[0] == 'L':  # linear penalty cost
                ins.customer_utilities[k] = ins.customer_usage[k] / 1000 * penalty_rate - ins.customer_charging_cost[
                    k]
            elif scenario[0] == 'Q':  # quadratic cost
                ins.customer_utilities[k] = (ins.customer_usage[k] / 1000 * penalty_rate) ** 2 - \
                                            ins.customer_charging_cost[k]

        # print(ins.customer_charging_options[k])
        ...
        #  if chg_length <= min_chg_length or chg_length + ins.customer_charge_start_at_time[
        #     k] >= ins.scheduling_horizon:
    ...
    # Hours_done[k] = json.dumps(data[k]["doneChargingTime"][17:19])
    # Hours_done[k] = Hours_done[k].strip('""')
    # Hours_done[k] = float(Hours_done[k])
    # Minutes_done[k] = json.dumps(data[k]["doneChargingTime"][20:22])
    # Minutes_done[k] = Minutes_done[k].strip('""')
    # Minutes_done[k] = float(Minutes_done[k])
    # StartSession=int((int((Hours/.25))+(Minutes+15)/15))
    # doneSession_done[k] = int((int((Hours_done[k] / .25)) + (Minutes_done[k] + 15) / 15))

    # ins.customer_charge_start_at_time[k] = np.floor(np.random.normal(charging_start_time_mean/step_length, charging_start_time_std))
    # while True:
    # chg_length[k] =ins.customer_charge_start_at_time[k]- doneSession_done[k] # in time steps
    # if chg_length+ins.customer_charge_start_at_time[k]>= ins.scheduling_horizon:
    ...
    # ins.customer_charging_cost[k] = np.sum(ins.cost_rate_matrix[t-1] for t in ins.customer_charging_time_path[k])
...
def sim_instance_ev_scheduling(scenario="L", n=10,
    ...
    # distribute demands
    soc = stats.truncnorm((initial_SOC_range[0] - SOC_mu) / SOC_std, (initial_SOC_range[1] - SOC_mu) / SOC_std, SOC_mu,
                          SOC_std)
    ins.customer_SOCs = soc.rvs(n)
    # print('1')
